...
# RCA Engine Demo Prerequisites Loader
# This script loads all tables, metadata, rules, and business logic for the RCA Engine demo
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     RCA Engine Demo Prerequisites Loader                  ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Step 1: Verify tables directory and CSV files
echo -e "${YELLOW}[1/6]${NC} Verifying tables directory and CSV files..."
TABLES_DIR="$SCRIPT_DIR/tables"
METADATA_DIR="$SCRIPT_DIR/metadata"

if [ ! -d "$TABLES_DIR" ]; then
    echo -e "${RED}❌ Tables directory not found: $TABLES_DIR${NC}"
    exit 1
fi

REQUIRED_TABLES=(
    "repayments.csv"
    "lmsdata_emi_payment_view.csv"
    "current_month_collection_report.csv"
    "outstanding_daily.csv"
    "da_orders.csv"
    "provisional_writeoff.csv"
    "writeoff_users.csv"
)
...
echo -e "${GREEN}✅ All ${#REQUIRED_TABLES[@]} tables found${NC}"
for table in "${REQUIRED_TABLES[@]}"; do
    SIZE=$(du -h "$TABLES_DIR/$table" | cut -f1)
    echo -e "   - $table ($SIZE)"
done
echo ""

# Step 2: Create metadata directory if it doesn't exist
echo -e "${YELLOW}[2/6]${NC} Setting up metadata directory..."
mkdir -p "$METADATA_DIR"
echo -e "${GREEN}✅ Metadata directory ready${NC}"
echo ""

# Step 3: Load comprehensive tables metadata
echo -e "${YELLOW}[3/6]${NC} Loading tables metadata..."
...
# Step 4: Load business rules with descriptions
echo -e "${YELLOW}[4/6]${NC} Loading business rules..."
...
echo -e "${YELLOW}[5/6]${NC} Loading lineage and join relationships..."
...
echo -e "${YELLOW}[6/6]${NC} Loading entity mappings..."
...
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}✅ Prerequisites loaded successfully!${NC}"
echo ""
...
echo -e "  1. Start backend: ${BLUE}cd backend && node server.js${NC}"
echo -e "  2. Start UI: ${BLUE}cd ui && npm run dev${NC}"
echo -e "  3. Access UI: ${BLUE}http://localhost:5173${NC}"
echo ""
echo -e "${GREEN}Demo prerequisites are ready! 🚀${NC}"
